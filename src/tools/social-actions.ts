...
} from '../schemas/social-actions.js';
...
async function getPersonUrn(client: LinkedInClient): Promise<string> {
  const userInfo = await client.get<{ sub: string }>('/userinfo', undefined, false);
 ...
 */
function parseCommentUrn(commentUrn: string): { activityUrn: string; commentId: string } {
  // Match urn:li:comment:(urn:li:activity:ACTIVITY_ID,COMMENT_ID)
  const fullMatch = commentUrn.match(
    /^urn:li:comment:\((urn:li:activity:\d+),(\d+)\)$/,
  );
  if (fullMatch) {
  ...
  const ugcMatch = commentUrn.match(
    /^urn:li:comment:\((urn:li:ugcPost:\d+),(\d+)\)$/,
  );
  if (ugcMatch) {
  ...
  throw new Error(
    `Invalid comment URN format: "${commentUrn}". Expected format: urn:li:comment:(urn:li:activity:ID,COMMENT_ID)`,
  );
}
 ...
 */
export function registerSocialActionTools(server: McpServer, client: LinkedInClient): void {
  // ─── get_post_comments ──────────────────────────────────────────────────
  server.registerTool(
    'get_post_comments',
    {
    ...
    async (params) => {
      try {
        ...
        const result = await client.get(
          ...
          { count: String(count), start: String(start) },
          false,
        );
  ...
  server.registerTool(
    ...
    async (params) => {
      try {
        ...
        await client.delete(
          `/socialActions/${encodedActivityUrn}/comments/${commentId}`,
          false,
        );
